...
{
  [TestFixture]
  public class ParallelWorkstationFixture : WorkstationBaseFixture<ParallelWorkstation>
  {
    protected override ParallelWorkstation CreateSUT()
    {
    ...
    public void Do_Not_Setup_But_Process_Products_If_ProductType_Stays_Same()
    {
      ...
      Assert.AreEqual(1, scheduler.GetCountForContainingProduct<ProcessingEndEvent>(productA2));
      Assert.AreEqual(1,
                      scheduler.OfType<IdleEndEvent>()
                               .Count());
      Assert.IsFalse(sut.IsAvailable(productB1));
