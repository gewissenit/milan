...
{
  [TestFixture]
  public class AssemblyStationFixture : WorkstationBaseFixture<AssemblyStation>
  {
    protected override AssemblyStation CreateSUT()
    {
      return new AssemblyStation();
    }

    protected override void Default_Ctor(AssemblyStation sut)
    {
    ...
    [Test]
    public void Do_Not_Setup_But_Transfor_Products_If_TransformationRule_Stays_Same()
    {
      var scheduler = new SpyScheduler();

      var experimentMock = _mockRepo.DynamicMock<IExperiment>();
      var modelMock = _mockRepo.DynamicMock<IModel>();
      var processingDurationDistributionAMock = _mockRepo.DynamicMock<IRealDistribution>();
      var inputAMock = _mockRepo.DynamicMock<IProductTypeAmount>();
      var inputBMock = _mockRepo.DynamicMock<IProductTypeAmount>();
      var outputCMock = _mockRepo.DynamicMock<IProductTypeAmount>();
      var transformationRuleAMock = _mockRepo.DynamicMock<ITransformationRule>();
      var transformationRuleBMock = _mockRepo.DynamicMock<ITransformationRule>();
      var transformationRuleAOutputMock = _mockRepo.DynamicMock<ITransformationRuleOutput>();
      var productTypeA = _mockRepo.DynamicMock<IProductType>();
      var productTypeB = _mockRepo.DynamicMock<IProductType>();
      var productTypeC = _mockRepo.DynamicMock<IProductType>();
      var productTypeD = _mockRepo.DynamicMock<IProductType>();
      ...
      successorMock.Expect(sm => sm.IsAvailable(productA1))
                   ...
                   .Return(true)
                   .Repeat.Twice();
      transformationRuleAMock.Expect(trm => trm.Inputs)
                             .Return(new[]
                                     {
                                       inputAMock
                                     });
      transformationRuleAMock.Expect(trm => trm.GetSampleOutput())
                             .Return(transformationRuleAOutputMock);
      transformationRuleBMock.Expect(trm => trm.Inputs)
                             .Return(new[]
                                     {
                                       inputBMock
                                     });
      transformationRuleAOutputMock.Expect(trm => trm.Outputs)
                                   .Return(new[]
                                           {
                                             outputCMock
                                           });
      transformationRuleAOutputMock.Expect(trm => trm.Distribution)
                                   .Return(processingDurationDistributionAMock);
      inputAMock.Expect(m => m.ProductType)
                .Return(productTypeA);
      inputAMock.Expect(m => m.Amount)
                .Return(1);
      ...
      outputCMock.Expect(m => m.Amount)
                 .Return(1);
      processingDurationDistributionAMock.Expect(rdm => rdm.GetSample())
                                         .Return(5);
      setupDurationDistributionMock.Expect(sddm => sddm.GetSample())
                                   .Return(5);
      experimentMock.Expect(x => x.Scheduler)
      ...
      sut.CurrentExperiment = experimentMock;
      sut.Model = modelMock;
      sut.AddTransformationRule(transformationRuleAMock);
      sut.AddTransformationRule(transformationRuleBMock);
      sut.SetupDurationDistribution = setupDurationDistributionMock;
      sut.HasSetup = true;
      sut.Add(connectionMock);
      ...
      Assert.AreEqual(1, scheduler.OfType<IdleEndEvent>()
                                  .Count());
      Assert.AreEqual(1, scheduler.GetCountForContainingProduct<TransformationStartEvent>(productA2));
      scheduler.ProcessNextSchedulable();
      scheduler.ProcessNextSchedulable();
      scheduler.ProcessNextSchedulable();
      _mockRepo.VerifyAll();
    ...
    [Test]
    public void Process_TranformationRule_With_Higher_Priority_First()
    {
      var scheduler = new SpyScheduler();
      ...
      var inputAMock = _mockRepo.DynamicMock<IProductTypeAmount>();
      var inputBMock = _mockRepo.DynamicMock<IProductTypeAmount>();
      var inputCMock = _mockRepo.DynamicMock<IProductTypeAmount>();
      var productTypeA = _mockRepo.DynamicMock<IProductType>();
      var productTypeB = _mockRepo.DynamicMock<IProductType>();
      var productA = MockProduct(modelMock, productTypeA);
      var productB = MockProduct(modelMock, productTypeB);
      var transformationRuleMock = _mockRepo.DynamicMock<ITransformationRule>();
      var transformationRuleWithMinorPriorityMock = _mockRepo.DynamicMock<ITransformationRule>();
      var transformationRuleOutputMock = _mockRepo.DynamicMock<ITransformationRuleOutput>();

      transformationRuleMock.Expect(trm => trm.Inputs)
                            .Return(new[]
                                    {
                                      inputBMock, inputCMock
                                    });
      ...
      transformationRuleMock.Expect(trm => trm.GetSampleOutput())
                            .Return(transformationRuleOutputMock);
      transformationRuleOutputMock.Expect(trm => trm.Outputs)
                                  .Return(new IProductTypeAmount[0]);
      transformationRuleWithMinorPriorityMock.Expect(trm => trm.Inputs)
                                             .Return(new[]
                                                     {
                                                       inputAMock
                                                     });
      transformationRuleWithMinorPriorityMock.Expect(trm => trm.Probability)
                                             .Return(1);
      inputAMock.Expect(m => m.ProductType)
                .Return(productTypeA);
      inputAMock.Expect(m => m.Amount)
      ...
      inputBMock.Expect(m => m.Amount)
                .Return(1);
      inputCMock.Expect(m => m.ProductType)
                .Return(productTypeA);
      inputCMock.Expect(m => m.Amount)
                .Return(1);
      experimentMock.Expect(x => x.Scheduler)
                    .Return(scheduler);
      ...
      sut.Model = modelMock;
      sut.AddTransformationRule(transformationRuleMock);
      sut.AddTransformationRule(transformationRuleWithMinorPriorityMock);

      Assert.IsFalse(sut.IsAvailable(productB));

      sut.Initialize();

      Assert.IsTrue(sut.IsAvailable(productB));
      sut.IsInState("Idle");
      Assert.AreEqual(1, scheduler.Count());
      Assert.AreEqual(1, scheduler.OfType<IdleStartEvent>()
                                  .Count());
      ...
      sut.Receive(productB);
      Assert.IsTrue(sut.IsAvailable(productA));
      sut.IsInState("Idle");
      Assert.AreEqual(1, scheduler.Count());
      Assert.AreEqual(1, scheduler.GetCountForContainingProduct<ProductReceiveEvent>(productB));

      scheduler.ProcessNextSchedulable();

      sut.Receive(productA);
      sut.IsInState("Processing");
      Assert.AreEqual(3, scheduler.Count());
      Assert.AreEqual(1, scheduler.OfType<IdleEndEvent>()
                                  .Count());
      Assert.AreEqual(1, scheduler.GetCountForContainingProduct<ProductReceiveEvent>(productA));
      Assert.AreEqual(1, scheduler.GetCountForContainingProduct<TransformationStartEvent>(productA));
      Assert.AreEqual(1, scheduler.GetCountForContainingProduct<TransformationStartEvent>(productB));

      _mockRepo.VerifyAll();
    }

    [Test]
    public void Process_TranformationRule_With_Lesser_Products_First()
    {
      var scheduler = new SpyScheduler();
      ...
      var transformationRuleMock = _mockRepo.DynamicMock<ITransformationRule>();
      var transformationRuleWithMinorPriorityMock = _mockRepo.DynamicMock<ITransformationRule>();
      var transformationRuleOutputWithPriorityMock = _mockRepo.DynamicMock<ITransformationRuleOutput>();
      ...
      transformationRuleWithMinorPriorityMock.Expect(trm => trm.Probability)
                                             .Return(1);
      transformationRuleWithMinorPriorityMock.Expect(trm => trm.GetSampleOutput())
                                             .Return(transformationRuleOutputWithPriorityMock);
      transformationRuleOutputWithPriorityMock.Expect(trm => trm.Outputs)
                                              .Return(new IProductTypeAmount[0]);
      inputAMock.Expect(m => m.ProductType)
                .Return(productTypeA);
      inputAMock.Expect(m => m.Amount)
      ...
      sut.AddTransformationRule(transformationRuleMock);
      sut.AddTransformationRule(transformationRuleWithMinorPriorityMock);
      Assert.IsFalse(sut.IsAvailable(productA));
    ...
    [Test]
    public void Setup_And_Then_Transfor_Products_If_TransformationRule_Changed()
    {
      var scheduler = new SpyScheduler();
