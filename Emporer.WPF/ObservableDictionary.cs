...
{
  [Serializable]
  public class ObservableDictionary<TKey, TValue> : IDictionary<TKey, TValue>, IDictionary, ISerializable,
                                                    IDeserializationCallback, INotifyCollectionChanged, INotifyPropertyChanged
  {
    ...
    protected readonly KeyedDictionaryEntryCollection<TKey> _keyedEntryCollection;
    ...
    public bool TryGetValue(TKey key, out TValue value)
    {
      ...
      value = result
                ? (TValue) _keyedEntryCollection[key].Value
                : default(TValue);
      return result;
    }
    ...
    protected class KeyedDictionaryEntryCollection<NestedTKey> : KeyedCollection<NestedTKey, DictionaryEntry>
    {
      #region constructors
      ...
      public KeyedDictionaryEntryCollection(IEqualityComparer<NestedTKey> comparer)
        : base(comparer)
      {
      }
      ...
      protected override NestedTKey GetKeyForItem(DictionaryEntry entry)
      {
        return (NestedTKey) entry.Key;
      }
    ...
    public struct Enumerator<NestedTKey, NestedTValue> : IEnumerator<KeyValuePair<NestedTKey, NestedTValue>>, IDictionaryEnumerator
    {
      internal Enumerator(ObservableDictionary<NestedTKey, NestedTValue> dictionary, bool isDictionaryEntryEnumerator)
      {
        _dictionary = dictionary;
        _version = dictionary._version;
        _index = -1;
        _isDictionaryEntryEnumerator = isDictionaryEntryEnumerator;
        _current = new KeyValuePair<NestedTKey, NestedTValue>();
      }

      public KeyValuePair<NestedTKey, NestedTValue> Current
      {
        get
        {
          ValidateCurrent();
      ...
      public bool MoveNext()
      {
        ...
        {
          _current = new KeyValuePair<NestedTKey, NestedTValue>((NestedTKey) _dictionary._keyedEntryCollection[_index].Key,
                                                                (NestedTValue) _dictionary._keyedEntryCollection[_index].Value);
          return true;
        }
        _index = -2;
        _current = new KeyValuePair<NestedTKey, NestedTValue>();
      ...
      object IEnumerator.Current
      {
        get
        {
          ...
          return new KeyValuePair<NestedTKey, NestedTValue>(_current.Key, _current.Value);
        }
      }
      ...
      private readonly ObservableDictionary<NestedTKey, NestedTValue> _dictionary;
      private readonly int _version;
      private int _index;
      private KeyValuePair<NestedTKey, NestedTValue> _current;
