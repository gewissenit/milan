...
using Emporer.Material.UI.Factories;
using Emporer.WPF;
...
namespace Emporer.Material.UI.ViewModels
{
  ...
  public class MaterialNavigatorViewModel : PropertyChangedBase
  {
    ...
    private void RemoveRoot(object model)
    {
      ...
      SelectedViewModelItem = RootItems.FirstOrDefault();
    }

    private void AddRoot(ICategory model)
    {
      var root = _categoryNavigationViewModelFactory.CreateNavigationViewModel(model);
      RootItems.Add(root);
      SelectedViewModelItem = root;
    }

    private void AddRoot(IMaterial model)
    {
      var root = _materialNavigationViewModelFactory.CreateNavigationViewModel(model);
      RootItems.Add(root);
      SelectedViewModelItem = root;
    }
