...
namespace EcoFactory.Components
{
  ...
  public class ProbabilityAssemblyStation : WorkstationBase, IProbabilityAssemblyStation
  {
    private ITransformationRule _currentTransformationRule;
    ...
    [JsonProperty]
    private IList<ITransformationRule> _TransformationRules = new List<ITransformationRule>();
    ...
    public IEmpiricalIntDistribution TransformationSelectionDistribution { get; set; }
    ...
    public override void Initialize()
    {
      ...
      PrepareTransformationSelectionDistribution();
    ...
    public override bool IsAvailable(Product product)
    {
      ...
      var tr = GetCurrentTransformationRule();
      var amount = 0;
      var productTypeAmount = tr.Inputs.SingleOrDefault(pta => pta.ProductType == product.ProductType);
      if (productTypeAmount != null)
      {
        amount = productTypeAmount.Amount;
      }
      return amount > _arrivedProducts.Count(p => p.ProductType == product.ProductType);
    }
    ...
    public override void Receive(Product product)
    {
      ...
      var transformationRule = GetCurrentTransformationRule();
      if (IsTransformationPossible(transformationRule))
      {
        _idle.Exit();
    ...
    public override void Reset()
    {
      ...
      _currentTransformationRule = null;
      TransformationSelectionDistribution = null;
      _lastOutput = null;
      _lastInput = null;
      _processing = null;
      _isInSetup = false;
    ...
    private ITransformationRule GetCurrentTransformationRule()
    {
      if (_currentTransformationRule == null)
      {
        var sample = TransformationSelectionDistribution.GetSample();
        _currentTransformationRule = _TransformationRules[sample];
      }
      return _currentTransformationRule;
    }

    private bool IsTransformationPossible(ITransformationRule tr)
    {
      return tr.Inputs.All(pta => _arrivedProducts.Count(p => p.ProductType == pta.ProductType) == pta.Amount);
    }

    private void PrepareTransformationSelectionDistribution()
    {
      _TransformationRules = _TransformationRules.OrderBy(tro => tro.Probability)
                                                 .ToList();

      var cumulativeFrequency = 0;
      foreach (var transformationRule in TransformationRules)
      {
        cumulativeFrequency += transformationRule.Probability;
        if (cumulativeFrequency > 100)
        {
          throw new ModelConfigurationException(Model, this, "The probabilities of a transformation rule is not well defined. The probabilities should be in sum 100.");
        }
        if (!TransformationSelectionDistribution.TryAddEntry(_TransformationRules.IndexOf(transformationRule), (double) cumulativeFrequency / 100))
        {
          throw new ModelConfigurationException(Model, this, "The probabilities of a transformation rule is not well defined. The probabilities should be in sum 100.");
        }
      }
